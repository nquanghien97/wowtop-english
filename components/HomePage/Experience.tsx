            ...
            {data.length === 0 ? (
              ...
              <div className="flex gap-4 flex-col md:flex-row">
                <div className="w-full md:w-2/5 flex flex-col">
                  ...
                  <div className="text-justify text-sm font-semibold text-[#3f3c3c] line-clamp-3-fallback md:line-clamp-4-fallback italic overflow-hidden" dangerouslySetInnerHTML={{ __html: data[0].content }} />
                  ...
                  {dataWithoutFirstLastItem.map(item => (
                    <div key={item.id} className="flex mb-4">
                      ...
                      <div className="flex flex-col w-1/2">
                        ...
                        <div className="text-justify line-clamp-3-fallback mr-4 text-sm font-semibold text-[#3f3c3c] italic overflow-hidden" dangerouslySetInnerHTML={{ __html: item.content }} />
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {data.length === 0 ? (
                ...
                dataWithoutLastItem.map(item => (
                  <li key={item.id} className="mb-4">
                    <Link href={`/tin-tuc/${item.slug}`} className="text-[#84571B] font-semibold mb-2 line-clamp-2-fallback">{item.title}</Link>
                    <div className="text-justify text-sm font-semibold text-[#3f3c3c] line-clamp-3-fallback italic overflow-hidden" dangerouslySetInnerHTML={{ __html: item.content }} />
                  </li>
                ))
              )}
            </ul>
          ...
          <div className="flex gap-4 max-md:flex-col max-lg:hidden">
            ...
            <div className="md:w-3/5 flex flex-col">
              <div className="flex-1">
                ...
                <div className="text-justify line-clamp-5-fallback text-sm font-semibold text-[#3f3c3c] italic" dangerouslySetInnerHTML={{ __html: data[data.length - 1].content }} />
              </div>
              <div className="flex">
                <Link className="mt-3 text-[#84571B] px-4 py-2 rounded-md border-[1px] border-[#065691] inline-block" href={`/tin-tuc`} >
                  Xem thêm
